...
if __name__ == "__main__":
    ...
    model_path = "model_data/Epoch100-Total_Loss18.4690-Val_Loss35.0919.pth"
    # 加快模型训练的效率
    ...
    """
    with open(annotation_path) as ftrain:
    	trainlines = ftrain.readlines()
    num_train = len(trainlines) 
    with open(val_path) as fval:
    	vallines =fval.readlines()
    num_val = len(vallines)
    ...
    if True:
        ...
        if Use_Data_Loader:
            train_dataset = YoloDataset(trainlines, (input_shape[0], input_shape[1]), mosaic=mosaic)
            val_dataset = YoloDataset(vallines, (input_shape[0], input_shape[1]), mosaic=False)
            gen = DataLoader(train_dataset, batch_size=Batch_size, num_workers=4, pin_memory=True,
                                    drop_last=True, collate_fn=yolo_dataset_collate)
            gen_val = DataLoader(val_dataset, batch_size=Batch_size, num_workers=4,pin_memory=True, 
                                    drop_last=True, collate_fn=yolo_dataset_collate)
        else:
            gen = Generator(Batch_size, trainlines,
                            (input_shape[0], input_shape[1])).generate(mosaic = mosaic)
            gen_val = Generator(Batch_size, vallines,
